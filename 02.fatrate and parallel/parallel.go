...
func main() {
	...
	if (x1 == x2 && x2 == y1 && y1 == y2) || (x3 == x4 && x4 == y3 && y3 == y4) {
		...
		if k1 == k2 {
			...
			//判断两条线段是否在一条直线方程上
			//一条直线方程上有重合的点
			if b1 == b2 && ((x1 > x2 && ((x3 >= x2 && x3 <= x1) || (x4 >= x2 && x4 <= x1))) || (x1 < x2 && ((x3 >= x1 && x3 <= x2) || (x4 >= x1 && x4 <= x2)))) {
				fmt.Println("两条直线不平行，且有重合")
			} else {
				fmt.Println("两条直线平行")
			}
		} else {
		...
		//判断两条线段是否在一条直线方程上
		//一条直线方程上有重合的点
		if x1 == x3 && ((y1 > y2 && ((y3 >= y2 && y3 <= y1) || (y4 >= y2 && y4 <= y1))) || (y1 < y2 && ((y3 >= y1 && y3 <= y2) || (y4 >= y1 && y4 <= y2)))) {
			fmt.Println("两条直线不平行,垂直于x轴且有重合")
		} else {
