...
func main() {
	for {
		...
		fmt.Scanln(&num)
		for i := 0; i < num; i++ {
			sexWeight = getMaterialsFromInput(i, names, ages, sexs, sexWeight, talls, weights)
			fatRateSum = calcFatRate(bmis, i, weights, talls, fatRates, ages, sexWeight, fatRateSum)
		}
		//判断体脂情况
		for j := 0; j < num; j++ {
...
func calcFatRate(bmis [999]float64, i int, weights [999]float64, talls [999]float64, fatRates [999]float64, ages [999]int, sexWeight int, fatRateSum float64) float64 {
	//计算体脂率
...
func getMaterialsFromInput(i int, names [999]string, ages [999]int, sexs [999]string, sexWeight int, talls [999]float64, weights [999]float64) int {
