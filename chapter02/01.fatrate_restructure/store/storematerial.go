...
import (
	"fmt"
	calc2 "learn.go/chapter02/01.fatrate_restructure/calc"
	"learn.go/chapter02/01.fatrate_restructure/input"
)
...
func StoreMaterial(num int) (names, sexs [999]string, bmis, fatRates [999]float64, ages [999]int, fatRateSum float64) {
	...
	for i := 0; i < num; i++ {
		...
		bmis[i] = calc2.CalcuBmi(weights[i], talls[i])
		fatRates[i] = calc2.CalcuFatrate(ages[i], sexWeights[i], bmis[i])
